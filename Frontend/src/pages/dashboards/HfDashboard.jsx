...
export default function HfDashboard() {
  ...
  const openVote = (brm, decision) => {
    setVoteModal({ brm, decision });
    setComments('');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
      ...
      {/* ─── NEW NAVIGATION TABS ─── */}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-orange-400">{pendingApprovals.length}</p>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            ...
            <p className="text-lg font-bold mt-1 text-orange-300">Head Functional</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {pendingApprovals.map((approval) => {
              ...
              const now = new Date();
              const hoursLeft = Math.max(0, Math.floor((dueDate - now) / (1000 * 60 * 60)));
              const isUrgent = hoursLeft < 12;

              return (
                <div key={approval.id}
                  className={`bg-slate-800 border rounded-xl p-5 ${isUrgent ? 'border-red-500/40' : 'border-slate-700'}`}>
                  {/* Card Header */}
                  ...
                  <div className="flex items-center gap-3 pt-3 border-t border-slate-700">
                    <button onClick={() => openVote(brm, 'APPROVED')}
                      className="flex-1 py-2 rounded-lg bg-green-500/15 hover:bg-green-500/25 border border-green-500/30 text-green-400 font-semibold text-sm transition-all flex items-center justify-center gap-2">
                    ...
                    <button onClick={() => openVote(brm, 'REJECTED')}
                      className="flex-1 py-2 rounded-lg bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 text-red-400 font-semibold text-sm transition-all flex items-center justify-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {voteModal && (
        <Modal
          title={voteModal.decision === 'APPROVED' ? '✓ Approve BRM' : '✕ Reject BRM'}
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">
                ...
                {voteModal.decision === 'APPROVED' && <span className="text-slate-500">(optional)</span>}
              </label>
              <textarea
                value={comments}
                onChange={e => setComments(e.target.value)}
                rows={3}
                placeholder={voteModal.decision === 'REJECTED' ? 'Required: Explain the reason for rejection...' : 'Optional: Add comments about your approval...'}
